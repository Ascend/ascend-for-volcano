...
func buildNpuNode(nodeName string, nodeArch string, cpu string, mem string, npuAllocatecNum string, npuTop string) *api.NodeInfo {
	...
	errs := addNodeSelector(v1node, nodeArch)
	if errs != nil {
...
func buildValidJobTests() []TestCaseStruct {
	...
	nodeStand1 := buildNpuNode(nodeName1, osArchArm, "192", "755Gi", "8",
		"Ascend910-0,Ascend910-1,Ascend910-2,Ascend910-3,Ascend910-4,Ascend910-5,Ascend910-6,Ascend910-7")
	nodeStand2 := buildNpuNode(nodeName2, osArchArm, "192", "755Gi", "8",
		"Ascend910-0,Ascend910-1,Ascend910-2,Ascend910-3,Ascend910-4,Ascend910-5,Ascend910-6,Ascend910-7")
	nodeFault1 := buildNpuNode(nodeName2, osArchArm, "192", "755Gi", "7",
		"Ascend910-0,Ascend910-1,Ascend910-2,Ascend910-3,Ascend910-5,Ascend910-6,Ascend910-7")
	nodeStand3 := buildNpuNode(nodeName1, osArchArm, "192", "755Gi", "8",
		"Ascend910-0")
...
func buildNpuPredicateTests() []TestCaseStruct {
	...
	npuNodeStand1 := buildNpuNode(nodeName1, osArchArm, "192", "755Gi", "8",
		"Ascend910-0,Ascend910-1,Ascend910-2,Ascend910-3,Ascend910-4,Ascend910-5,Ascend910-6,Ascend910-7")
	stableNode := buildNpuNode(nodeName2, osArchArm, "192", "755Gi", "8",
		"Ascend910-0,Ascend910-1,Ascend910-2,Ascend910-3,Ascend910-4,Ascend910-5,Ascend910-6")
	cpuNodeStand1 := buildNpuNode(nodeName1, osArchArm, "192", "755Gi", "0", "")
...
func addNpuNodePool(nodeName string, npuAllocatecNum int, nodePool map[string]*api.NodeInfo) {
	...
	for leftHccsNum := 0; leftHccsNum <= npuNumPerHccs; leftHccsNum++ {
		for rightHccsNum := 0; rightHccsNum <= npuNumPerHccs; rightHccsNum++ {
			...
			p := buildNpuNode(nodeName, osArchArm, "96", "64Gi", strconv.Itoa(npuAllocatecNum), annotation)
			nodePool[key] = p
		}
	}
}
		...
		for _, task := range jobInfo.Tasks {
			...
			if expectScore := expType[taskID][task.NodeName]; expectScore != score[task.NodeName] {
				fmt.Println("case " + strconv.Itoa(testNo) + " : " + test.name + " task:" +
					taskID + " on node:" + task.NodeName + " test failed, expect have score:" + strconv.Itoa(int(expectScore)) +
