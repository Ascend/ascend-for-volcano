...
func validJobModel(job *api.JobInfo) error {
	...
	if jobNPU, err = util.GetJobReqNPUNum(job, npu800And9000CardName); err != nil {
		return err
	}
...
func validJobNPUNum(job *api.JobInfo) error {
	jobNPU, err := util.GetJobReqNPUNum(job, npu800And9000CardName)
	if err != nil {
...
func isMyJob(job *vapi.JobInfo) error {
	_, err := util.GetJobReqNPUNum(job, npu800And9000CardName)
	if err != nil {
