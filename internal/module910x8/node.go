...
func getNodeNPUNumFromOthers(nodeInfo *api.NodeInfo) (int, error) {
	top := util.GetTopFromNodeOthers(nodeInfo, npu800And9000CardName, npu910CardPreName)
	if top == nil {
...
func checkNPUResourceStable(node *vapi.NodeInfo) error {
	...
	nodeNPUIdleNumFromIdle, err := util.GetNodeNPUNumFromIdle(node, npu800And9000CardName)
	if err != nil {
...
func isMyNode(node *vapi.NodeInfo) error {
	_, err := util.GetNPUAllocCardsFromNodeOthers(node, npu800And9000CardName)
	if err != nil {
...
func getUsableTopFromNode(node *api.NodeInfo, distributeFlag bool) []int {
	nodeNPUTopology := util.GetTopFromNodeOthers(node, npu800And9000CardName, npu910CardPreName)
	if !distributeFlag {
