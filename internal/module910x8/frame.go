...
func (tp *module910x8) PreCheckNodeFn(task *vapi.TaskInfo, node *vapi.NodeInfo, confs []conf.Configuration) error {
	if rescheduling.IsNodeInFaultNodeList(node) {
		return fmt.Errorf("pre check node function %s in fault node list", node.Name)
	}
	...
	if err := util.IsSelectorMeetNode(task, node, schedulerConf, npu800And9000CardName); err != nil {
		// get scheduler selector configure failed, but need continue
...
func (tp *module910x8) ScoreBestNPUNodesFn(scoreMap map[string]float64,
	...
	for nodeName, priority := range bestNodes {
		healthNPUNumber, err := util.GetNodeHealthNPUNumberByName(nodeName, nodes, npu800And9000CardName)
		if err != nil {
...
func (tp *module910x8) GetAllocatedNPUFromTopologyFn(task *vapi.TaskInfo,
	...
	taskNPUNumber, taskError := util.GetTaskNPUNum(task, npu800And9000CardName)
	if taskError != nil {
...
func (tp *module910x8) SetNPUTopologyToPodFn(task *vapi.TaskInfo, top interface{}) error {
	...
	topologyStr = util.ChangeIntArrToStr(intTop, npu910CardPreName)
...
func (tp *module910x8) UpdateNPUNodeUsedCardFn(node *vapi.NodeInfo, top interface{}) error {
	...
	nodeDeviceIDs := util.GetTopFromNodeOthers(node, npu800And9000CardName, npu910CardPreName)
	...
	newNodeTopStr := util.GetRealTopAfterAlloc(nodeDeviceIDs, useTop, npu910CardPreName)
	if newNodeTopStr == "" {
		klog.V(logDebugLev).Infof("%s getRealTopAfterAlloc all top has allocated .", PluginName)
	}

	err := util.ReloadNewTopToNodeOther(node, newNodeTopStr, npu800And9000CardName)
	if err != nil {
...
func (tp *module910x8) GetReleaseNPUTopologyFn(task *vapi.TaskInfo) (interface{}, error) {
	// get task use top
	taskDevIDs := util.GetDeviceIDsFromAnnotations(task.Pod.Annotations, npu800And9000CardName, npu910CardPreName)
	if taskDevIDs == nil {
...
func (tp *module910x8) UpdateReleaseNPUNodeTopologyFn(node *vapi.NodeInfo, top interface{}) error {
	...
	// delete the use top
	newNodeTopStr := util.GetRealTopAfterRelease(nodeDeviceIDs, taskDeviceIDs, npu910CardPreName)
	if newNodeTopStr == "" {
...
func (tp *module910x8) IsMyTask(task *vapi.TaskInfo) error {
	_, err := util.GetTaskNPUNum(task, npu800And9000CardName)
	if err != nil {
...
func setGraceOverTime(ssn *framework.Session) error {
	...
	configuration, err := util.GetConfigFromSchedulerConfigMap(util.CMInitParamKey, ssn.Configurations)
	if err != nil {
