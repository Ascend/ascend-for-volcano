...
func (tp *chip310x4) getSelectedCardTop(taskNPUNumber int, priorityArray [cardNPUNumber]int, cardNumGroups [][]int,
	...
	for _, arrLen := range priorityArray {
		...
		if curGroupNum == 0 || arrLen == 0 {
			continue
		}
		// curGroupNum less or equal taskNPUNumber: use all the chips in the group
		// curGroupNum large taskNPUNumber: random use part of the card chip which the idea of perfect shuffling.
