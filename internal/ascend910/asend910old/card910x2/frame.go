...
func (tp *card910x2) PreStartAction(ssn *framework.Session) error {
	...
	if getRunErr != nil {
		klog.V(util.LogDebugLev).Infof("%s GetRunningJobs: %s", cardFullName, getRunErr.Error())
	}
	...
	if err != nil {
		klog.V(util.LogErrorLev).Infof("%s AddFaultJobWithSession %s", cardFullName, err)
	}
