...
func (tp *card310x4) PreCheckNodeFn(task *api.TaskInfo, node *api.NodeInfo, confs []conf.Configuration) error {
	...
	if err := util.IsSelectorMeetNode(task, node, schedulerConf, a310NPUCardName); err != nil {
		// get scheduler selector configure failed, but need continue
		klog.V(util.LogDebugLev).Infof("%s %s check %s : %v.", PluginName, task.Name, node.Name, err)
		return err
	}
	return nil
}
...
func (tp *card310x4) ScoreBestNPUNodesFn(scoreMap map[string]float64,
	...
	if len(scoreMap) == 0 || reflect.ValueOf(scoreMap).IsNil() {
		err := errors.New("score map is nil")
		klog.V(util.LogInfoLev).Infof("ScoreBestNPUNodesFn %s %v", PluginName, err)
		return nil, err
	}

	// the score value ranges(1 to 32), 4 is best.
...
func (tp *card310x4) GetAllocatedNPUFromTopologyFn(
	...
	if nodeTop == nil {
		msg := fmt.Errorf("not npu node[%s], no need to continue", node.Name)
		klog.V(util.LogDebugLev).Infof(msg.Error())
		return nil, msg
	}
	...
	for _, priority := range priorityArray {
		for _, cardNumGroup := range cardNumGroups {
			if priority == len(cardNumGroup) {
				allocTopologyNPUs := cardNumGroup[:taskNPUNumber]
