...
func (tp *ascend310P) CheckNodeNPUByTask(task *api.TaskInfo, node plugin.NPUNode) error {
	klog.V(util.LogDebugLev).Infof("%s CheckNodeNPUByDyTask job(%s).", tp.GetPluginName(), tp.Name)
	var err error
	...
	default:
		err = fmt.Errorf("%s no type %d", tp.Name, tp.Type)
		klog.V(util.LogDebugLev).Infof("%s CheckNodeNPUByDyTask %s %s.", tp.GetPluginName(), tp.Name, err)
		return err
	}
